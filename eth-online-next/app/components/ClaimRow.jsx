...
const ClaimRow = ({ready_for_claim,claim_tx_hash}) => {
...
`;
    ...
    const fetchGraph = useCallback(async () => {
        try {
            const res = await axios.post(apiUrl, { query,  variables: { ownerAddress } });
  ...
  return (
    <>
    {isAuthenticated && (claim_tx_hash ? <h1>Transaction has been claimed already</h1> : 
    ready_for_claim ? <button onClick={pollForClaimStatus}>Click to claim (button enabled)</button> :
    <button onClick={pollForClaimStatus}>Waiting for txnx to be completed. (button disabled)</button>)}
    </>
  )
}
